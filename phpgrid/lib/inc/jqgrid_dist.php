...
class jqgrid
{
	...
	function render($grid_id)
	{
		...
		if (isset($_POST['oper']))
		{
			...
			switch($op)
			{
				...
				case "del":
					...
					$sql2 = "DELETE FROM {$this->table} WHERE subhead_id <> 0 and subhead_id is not Null and type <> 2 and type <> 0 and subhead_id NOT IN 
							(SELECT nv  FROM 
		...
		if ($this->actions["rowactions"] !== false)
		{
			...
			if (!$defined)
			{
				$this->options["colNames"][] = _("Actions");
		...
		if (empty($this->options["add_options"]["success_msg"]))
			$this->options["add_options"]["success_msg"] = "Record added";// <meta http-equiv='refresh' content='1'>
		...
		if (empty($this->options["delete_options"]["success_msg"]))
			$this->options["delete_options"]["success_msg"] = "Record deleted";// <meta http-equiv='refresh' content='1'>
